...
class Box:
    ...
    def _setBox(self) -> None:
        if self._type == "inventory":
            self._box = pygame.image.load("assets/dialogue/inventory.png")
            self._box = pygame.transform.scale_by(self._box, (X_RATIO, Y_RATIO))
        else:
            self._box = pygame.image.load("assets/dialogue/static.png")
...
class Text:
    ...
    def __init__(
        ...
        if from_file:
            ...
            self._text = text
        self._font = pygame.font.Font(f"assets/font/{font_name}.ttf")
    ...
    def _getText(self) -> str:
        text_file = open(f"assets/dialogue/{self._path}.txt")
        text = text_file.read()
        text_file.close()
        return text
